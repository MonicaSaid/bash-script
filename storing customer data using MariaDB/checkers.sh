...
function checkUserExit {
    	  ID=${1}
	  RES=$(mysql -u ${DBUSER} -p${DBPASS} -h ${DBHOST} -e "select * from monica.customers where id=${ID};")
          if [ -z "${RES}" ]
          then
                  return 0
          else
...
function checkEmailExit {
	    EMAIL=${1}
	    FLINE=$(mysql -u ${DBUSER} -p${DBPASS} -h ${DBHOST} -e "select * from monica.customers where email='${EMAIL}';")
            [ -z "${FLINE}" ] && return 0
	    return 1
}
